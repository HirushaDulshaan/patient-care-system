...
const FinancialReports = () => {
  ...
  const summary = [
    ...
    {
      label: "Doctor Salaries (Expenses)",
      value: `Rs. ${stats?.totalExpenses.toLocaleString()}`,
      change: "+10.2%",
      trend: "up",
    },
    {
      label: "Net Profit (Hospital)",
    ...
    <div className="space-y-8 animate-in fade-in duration-700 font-sans">
        ...
        {summary.map((item, i) => (
          ...
          >
            ...
            <div className="mt-4 flex items-center gap-1.5 text-[10px] font-black text-emerald-500">
              ...
              <span>
                {item.change} vs Last {reportType}
              </span>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
          <h4 className="text-xl font-black text-slate-800 mb-10">
            ...
            <div className="absolute inset-0 flex items-end justify-around px-10 pb-6">
              {[30, 50, 45, 80, 65, 90, 75].map((h, i) => (
                <div
                  key={i}
                  style={{ height: `${h}%` }}
                  className="w-8 bg-blue-600 rounded-t-xl opacity-20 hover:opacity-100 transition-all cursor-pointer group"
                >
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[8px] font-black px-2 py-1 rounded opacity-0 group-hover:opacity-100">
                    Rs.{h}k
                  </div>
                </div>
              ))}
            </div>
            <p className="text-slate-300 font-black text-xs z-10 uppercase tracking-[0.3em]">
        ...
        <div className="lg:col-span-4 bg-slate-900 p-10 rounded-[3.5rem] text-white shadow-2xl">
          <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-10">
            Budget Allocation
          </h4>
            ...
            <tbody className="divide-y divide-slate-50">
              {stats?.recentTransactions.map((inv: any, i: number) => (
                <tr key={i} className="hover:bg-slate-50/50 transition">
                  ...
                  <td className="p-6 text-sm font-black text-blue-600">
                    {inv.amount}.00
                  </td>
                  <td className="p-6 text-right">
                    <span className="px-3 py-1 rounded-lg text-[9px] font-black uppercase bg-emerald-50 text-emerald-600">
                      SUCCESS
                    </span>
                  </td>
                </tr>
