...
const SuperAdminStaffManagement = () => {
  ...
  const [approveAccess, { loading: approveLoading }] = useMutation(
    APPROVE_STAFF,
    {
      ...
      onCompleted: () => {
        toast.success("Staff Access Approved & Password Assigned! ✅");
  ...
  const handleApproveSubmit = async () => {
    ...
    try {
      ...
      toast.error("Error occurred while approving staff access.");
    }
  };
  ...
  const handleStatusChange = async (userId: string, currentStatus: boolean) => {
    try {
      ...
      toast.error("Error occurred while toggling staff status.");
    }
  };
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Review registrations and grant system access permissions.
          </p>
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="p-6">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-2 h-2 rounded-full ${staff.user.isActive ? "bg-emerald-500" : "bg-amber-500"}`}
                        ></div>
                        <span
                          className={`text-[10px] font-black uppercase ${staff.user.isActive ? "text-emerald-600" : "text-amber-600"}`}
                        >
                    ...
                    <td className="p-6 text-right">
                      {!staff.user.password ? (
                        ...
                        <div className="flex items-center justify-end gap-3">
                          ...
                          >
                            <div
                              className={`absolute top-1 bg-white w-4 h-4 rounded-full transition-all ${staff.user.isActive ? "left-7" : "left-1"}`}
                            ></div>
                          </button>
                        </div>
                      )}
          ...
          <div className="bg-white w-full max-w-md rounded-[3.5rem] p-12 shadow-2xl animate-in zoom-in-95 duration-200 border border-white">
            <div className="text-center mb-10">
              ...
              <p className="text-slate-400 text-sm mt-2 font-medium">
                Create a secure password for <b>{selectedUser?.email}</b>
              </p>
            </div>

            <div className="space-y-6">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-4 mb-3 block">
