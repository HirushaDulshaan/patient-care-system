...
import { 
  Search, History, FileText, Pill, Calendar, 
  User, ArrowRight, Activity, Loader2, AlertCircle 
} from 'lucide-react';
import { gql } from '@apollo/client';
import { useLazyQuery } from '@apollo/client/react';

// 1. රෝගියාගේ නමෙන් හෝ NIC එකෙන් සර්ච් කරලා එයාගේ සම්පූර්ණ විස්තර සහ ඉතිහාසය ගන්නා Query එක
...
const PatientConsultation = () => {
  ...
  // 2. Apollo Lazy Query
  const [fetchRecords, { data, loading, error }] = useLazyQuery(GET_PATIENT_RECORDS);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (nicSearch.trim()) {
      ...
      setSelectedRecord(null); // අලුත් කෙනෙක් සර්ච් කරද්දී පරණ සිලෙක්ෂන් අයින් කරනවා
    }
  };
    ...
    <div className="space-y-6 font-sans animate-in fade-in duration-500">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <form onSubmit={handleSearch} className="relative w-full md:w-1/2">
          ...
          />
          {loading && <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 animate-spin text-blue-500" size={20} />}
        </form>
        ...
        {patientData ? (
          <div className="flex items-center gap-6 bg-slate-900 p-4 rounded-[2rem] text-white w-full md:w-auto shadow-xl">
             ...
             <div>
                <h3 className="font-bold text-sm leading-none">{patientData.fullName}</h3>
                <p className="text-[10px] font-medium opacity-70 mt-1">NIC: {patientData.nic} • {patientData.phone}</p>
             </div>
             <div className="h-8 w-[1px] bg-white/10 mx-2"></div>
             <div className="text-right">
                <p className="text-[9px] font-black uppercase opacity-60 tracking-widest">Total Visits</p>
                <p className="text-sm font-black text-blue-400">{medicalHistory.length}</p>
             </div>
          </div>
        ) : (
      ...
      {error && <div className="p-4 bg-red-50 text-red-500 rounded-2xl text-xs font-bold">Error loading history: {error.message}</div>}
           ...
           <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {medicalHistory.length > 0 ? (
                medicalHistory.map((record: any) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                       <span className={`text-[10px] font-black uppercase tracking-widest ${selectedRecord?.id === record.id ? 'text-white' : 'text-slate-400'}`}>
                          {new Date(record.createdAt).toLocaleDateString()}
                       </span>
                       <ArrowRight size={14} className={selectedRecord?.id === record.id ? 'text-white' : 'text-slate-300'} />
                    </div>
                    <p className={`font-bold text-sm line-clamp-1 ${selectedRecord?.id === record.id ? 'text-white' : 'text-slate-800'}`}>{record.diagnosis}</p>
                    <p className={`text-[10px] mt-1 ${selectedRecord?.id === record.id ? 'text-blue-100' : 'text-slate-500'}`}>Weight: {record.weight || 'N/A'}</p>
                  </button>
                ))
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-slate-300 p-10 text-center">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest leading-relaxed">No records found for this patient</p>
                </div>
              )}
           </div>
        </div>
           ...
           {selectedRecord ? (
             <div className="flex-1 overflow-y-auto p-10 animate-in slide-in-from-right-4 duration-500">
                ...
                <div className="flex justify-between items-start mb-10">
                   <div>
                      <h4 className="text-3xl font-black text-slate-900 tracking-tight">{selectedRecord.diagnosis}</h4>
                      <div className="flex items-center gap-3 mt-2">
                         <span className="flex items-center gap-1.5 text-xs font-bold text-slate-400">
                           <Calendar size={14}/> {new Date(selectedRecord.createdAt).toLocaleString()}
                         </span>
                         <span className="w-1.5 h-1.5 rounded-full bg-slate-200"></span>
                         <span className="text-xs font-bold text-blue-600">Age: {selectedRecord.age}</span>
                      </div>
                   </div>
                ...
                <div className="space-y-8">
                   <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                      <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                         <Activity size={14} className="text-blue-600" /> Patient Complaint
                      </h5>
                      <p className="text-slate-700 leading-relaxed font-medium">{selectedRecord.complaint}</p>
                   </div>
                         ...
                         {selectedRecord.prescriptions.map((med: any, i: number) => (
                           <div key={i} className="flex items-center justify-between p-4 bg-emerald-50 border border-emerald-100 rounded-2xl">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <p className="text-sm font-bold text-emerald-800">{med.drugName}</p>
                                   <p className="text-[10px] text-emerald-600 font-medium">{med.dosage}</p>
                                </div>
                             </div>
             ...
             <div className="flex-1 flex flex-col items-center justify-center text-slate-300 p-20 text-center">
                ...
                <h4 className="text-xl font-black uppercase tracking-widest">Medical Archive</h4>
                <p className="text-sm font-medium mt-2 italic">Select a visit record to view clinical details.</p>
             </div>
           )}
        </div>
