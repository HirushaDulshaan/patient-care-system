...
import {
    ...
    ArrowUpRight, Clock, ChevronRight, Loader2
} from 'lucide-react';

// 1. අද දවසේ පෝලිම (Today's Queue) ලබාගැනීමට
...
// 2. මාසික වාර්තා සහ ආදායම ලබාගැනීමට
...
const DoctorDashboard = () => {
    ...
    const { data: queueData, loading: queueLoading } = useQuery(GET_DOCTOR_STATS, {
    ...
    if (queueLoading || statsLoading) return (
        ...
        <div className="min-h-screen bg-[#F8FAFC] flex font-sans">
            <div className="flex-1 p-10">
                ...
                <header className="flex justify-between items-center mb-12">
                    <div>
                        <h2 className="text-4xl font-black text-slate-900 tracking-tight">Doctor's Console</h2>
                        <p className="text-slate-400 font-bold mt-1 uppercase text-xs tracking-widest flex items-center gap-2">
                            <TrendingUp size={14} className="text-emerald-500" /> Clinical Performance Overview
                        </p>
                    </div>
                    <div className="flex items-center gap-4 bg-white p-3 pr-6 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100">
                        <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-xl">D</div>
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Medical Officer</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                    ...
                    <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 relative group transition-all hover:scale-[1.02]">
                        ...
                        <p className="text-slate-400 text-[10px] uppercase font-black tracking-[0.2em]">Today's Queue</p>
                        <div className="flex items-baseline gap-2">
                            <h3 className="text-5xl font-black text-slate-800">{todayCount}</h3>
                            <span className="text-xs font-bold text-slate-400 italic">Patients Waiting</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 relative group transition-all hover:scale-[1.02]">
                        ...
                        <p className="text-slate-400 text-[10px] uppercase font-black tracking-[0.2em]">Monthly Volume</p>
                        <div className="flex items-baseline gap-2">
                            <h3 className="text-5xl font-black text-slate-800">{stats?.monthlyCount || 0}</h3>
                            <span className="text-xs font-bold text-slate-400 italic">Completed Visits</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-900 p-8 rounded-[3rem] shadow-2xl shadow-blue-900/20 relative group transition-all hover:scale-[1.02] overflow-hidden">
                        ...
                        <p className="text-slate-500 text-[10px] uppercase font-black tracking-[0.2em]">Estimated Revenue</p>
                        <div className="flex items-baseline gap-2">
                            <h3 className="text-4xl font-black text-white">LKR {(stats?.monthlyIncome || 0).toLocaleString()}</h3>
                        </div>
                        <p className="text-[9px] text-blue-400/60 font-bold mt-2 uppercase tracking-widest">Base: 2,000 Per Patient</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[4rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
                    <div className="p-10 border-b border-slate-50 flex justify-between items-center">
                        <div>
                            <h4 className="text-2xl font-black text-slate-800 tracking-tight">Active Appointments</h4>
                            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Pending Consultations for Today</p>
                        </div>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {todayAppointments.length > 0 ? todayAppointments.map((app: any) => (
                                    <tr key={app.id} className="hover:bg-blue-50/30 transition group rounded-3xl">
                                        <td className="p-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-black text-slate-800 text-sm">{app.patient.fullName}</p>
                                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">NIC: {app.patient.nic}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-6">
                                        ...
                                        <td className="p-6 text-xs font-black text-slate-500">{app.patient.phone}</td>
                                        <td className="p-6">
                                    ...
                                    <tr>
                                        <td colSpan={5} className="p-20 text-center text-slate-300 font-black uppercase tracking-[0.3em] text-xs italic">
