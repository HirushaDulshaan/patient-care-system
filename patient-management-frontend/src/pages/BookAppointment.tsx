      ...
      <div className="max-w-7xl mx-auto">
            ...
            {[1, 2, 3].map((s) => (
              <React.Fragment key={s}>
                <div className="flex items-center gap-3">
                  <div
                    className={`w-12 h-12 rounded-[1.25rem] flex items-center justify-center font-black transition-all duration-500 ${step >= s ? "bg-blue-600 text-white scale-110 shadow-lg shadow-blue-200" : "bg-slate-100 text-slate-400"}`}
                  >
                    {s === 3 && step === 3 ? <CheckCircle2 size={20} /> : s}
                  </div>
                  <span
                    className={`hidden md:block text-[10px] font-black uppercase tracking-widest ${step >= s ? "text-blue-600" : "text-slate-300"}`}
                  >
                ...
                {s < 3 && (
                  <div
                    className={`w-12 h-1 rounded-full ${step > s ? "bg-blue-600" : "bg-slate-100"}`}
                  ></div>
                )}
              </React.Fragment>
            ))}
          ...
          <div className="max-w-2xl mx-auto bg-white/70 backdrop-blur-xl p-12 rounded-[4rem] shadow-2xl border border-white animate-in fade-in zoom-in-95 duration-700">
            <div className="text-center mb-12">
              ...
              <p className="text-slate-400 text-sm mt-3 font-medium">
                Please provide your details to access our world-class
                specialists.
              </p>
            </div>
            ...
            <div className="space-y-6">
              <div className="group transition-all">
                <p className="text-[10px] font-black text-slate-400 uppercase ml-4 mb-2 tracking-widest">
                  Full Legal Name
                </p>
                <input
                  name="fullName"
                  ...
                  className="w-full p-6 bg-white border-2 border-slate-50 rounded-[2rem] font-bold outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all"
                />
              </div>
              ...
              <button
                disabled={!patientInfo.fullName || !patientInfo.nic}
                onClick={() => setStep(2)}
                className="w-full bg-slate-900 text-white py-6 rounded-[2.5rem] font-black uppercase tracking-widest mt-8 shadow-2xl hover:bg-blue-600 transition-all active:scale-95 disabled:opacity-30 flex items-center justify-center gap-3"
              >
        ...
        {step === 2 && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-700">
            <div className="flex flex-col lg:flex-row gap-6 items-center bg-white/50 backdrop-blur-md p-4 rounded-[3rem] border border-white/50">
              <div className="flex-1 w-full relative">
                ...
                <input
                  ...
                  className="w-full pl-16 pr-6 py-5 bg-white rounded-[2rem] outline-none font-bold text-sm shadow-sm focus:ring-4 focus:ring-blue-500/5 transition-all"
              ...
              <div className="flex gap-4 w-full lg:w-auto">
                <select
                  ...
                  className="bg-white px-8 py-5 rounded-[2rem] font-black text-[10px] uppercase outline-none shadow-sm cursor-pointer hover:bg-slate-50 transition-colors"
                >
                ...
                <input
                  type="date"
                  className="bg-white px-6 py-5 rounded-[2rem] font-bold text-xs outline-none shadow-sm cursor-pointer"
              ...
              {docLoading ? (
                ...
                docData?.getAllDoctors
                  ...
                  .map((doc: any) => {
                    ...
                    const relevantSlots =
                      doc.schedules?.filter((s: any) => {
                        const slotDate = new Date(s.workingDate)
                          .toISOString()
                          .split("T")[0];
                    ...
                    const isFull =
                      ...
                      firstSlot.remainingSeats <= 0;

                    const currentBooked = firstSlot ? firstSlot.bookedCount : 0;
                    const currentRemaining = firstSlot
                      ? firstSlot.remainingSeats
                      : 20;
                      ...
                      <div
                        ...
                        className={`group relative bg-gradient-to-br from-white to-slate-50/50 rounded-[4rem] p-10 border-2 transition-all duration-500 flex flex-col h-[500px] ${
                          isFull
                            ...
                            : "border-white hover:border-blue-500 hover:shadow-2xl cursor-pointer"
                        }`}
                      >
                        <div className="flex justify-between items-center mb-8">
                          <div
                            className={`w-20 h-20 rounded-[2rem] flex items-center justify-center ${isFull ? "bg-slate-200" : "bg-blue-600 text-white shadow-xl"}`}
                          >
                            <Stethoscope size={36} />
                          </div>
                          <div
                            className={`px-5 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest ${isFull ? "bg-slate-200 text-slate-500" : "bg-emerald-100 text-emerald-600"}`}
                          >
                            {isFull
                              ? "Session Full"
                              : `${currentRemaining} Seats Left`}
                          </div>
                        </div>
                          ...
                          <div className="mt-10 grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-white/50 p-4 rounded-3xl border border-white">
                              ...
                              <div className="flex items-center gap-2 text-slate-700 font-bold text-xs">
                                ...
                                {firstSlot
                                  ? `${currentBooked}/${firstSlot.maxPatients}`
                                  : "0/20"}
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="mt-10 flex justify-between items-center pt-8 border-t border-slate-100/50">
                          ...
                          <div
                            className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center transition-all ${isFull ? "bg-slate-100 text-slate-300" : "bg-slate-900 text-white"}`}
                          >
                            <ArrowRight size={28} />
                          </div>
                        </div>
            ...
            <button
              onClick={() => setStep(1)}
              className="flex items-center gap-2 text-slate-400 font-black text-[10px] uppercase mx-auto mt-12 hover:text-blue-600 transition-colors tracking-widest group"
            >
              <ChevronLeft
                size={16}
            ...
            <div className="bg-white rounded-[5rem] overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] border border-white">
              <div className="bg-slate-900 p-12 text-white flex flex-col md:flex-row justify-between items-center gap-8">
                <div className="text-center md:text-left">
                  ...
                  <p className="text-slate-400 font-bold mt-2 uppercase text-xs tracking-[0.3em]">
                    Final Step to Secure Appointment
                  </p>
                </div>
                    ...
                    <div className="flex items-center gap-5 p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-slate-400">
                          NIC: {patientInfo.nic} | TEL: {patientInfo.phone}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-2xl font-black tracking-tight">
                          {new Date(selectedSlot?.workingDate).toDateString()}
                        </span>
                      </div>
                      <div className="flex items-center gap-4">
                        <Clock size={20} className="text-blue-200" />
                        <span className="text-2xl font-black tracking-tight">
                          {selectedSlot?.startTime} Onwards
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-16 pt-0 flex flex-col md:flex-row gap-6">
                <button
                  onClick={() => setStep(2)}
                  className="flex-1 py-7 bg-slate-50 text-slate-400 font-black rounded-[2.5rem] uppercase text-[10px] tracking-widest hover:bg-slate-100 transition-all border border-slate-100"
                >
                  Change Selection
                </button>
                <button
                  ...
                  className="flex-[2] py-7 bg-blue-600 text-white font-black rounded-[2.5rem] shadow-2xl hover:bg-blue-700 uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 transition-all active:scale-95"
                >
                  {paymentLoading ? (
                    <Loader2 className="animate-spin" />
                  ) : (
                    <>
                      Secure Payment & Confirm <ArrowRight size={20} />
                    </>
                  )}
                </button>
              </div>
