    ...
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      <div className="w-80 bg-slate-950 text-white p-8 hidden md:flex flex-col sticky top-0 h-screen shadow-2xl">
        ...
        <div className="flex items-center gap-3 mb-12 px-2">
          ...
          <div>
            <h1 className="text-lg font-black tracking-tighter leading-none">SUPER ADMIN</h1>
            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Hospital Control</p>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-2">
          {menuItems.map((item) => (
            ...
            >
              ...
              {isActive(item.path) && <ChevronRight size={14} className="animate-in slide-in-from-left-2" />}
            </Link>
          ))}
        </nav>
