...
import { 
  LayoutDashboard, UserRound, ClipboardList, Clock, 
  Settings, LogOut, Bell, Search, Loader2 
} from 'lucide-react';

// 1. Sidebar එකේ පෙන්වන්න දොස්තරගේ නම සහ Category එක ගන්නා Query එක
...
const DoctorLayout = () => {
 ...
 const handleLogout = () => {
  // 1. සියලුම දත්ත අයින් කරන්න
  localStorage.clear(); 
  sessionStorage.clear(); // අවශ්‍ය නම් මෙයත් කරන්න

  // 2. Navigation history එක replace කරන්න (ආපහු back එන්න බැරි වෙන්න)
      ...
      <div className="w-72 bg-white border-r border-slate-200 hidden md:flex flex-col sticky top-0 h-screen">
        ...
        <div className="p-8">
          <div className="flex items-center gap-3 transition-transform hover:scale-105 cursor-pointer">
            ...
            <h1 className="text-lg font-black tracking-tighter text-slate-800 uppercase">Medic-OS</h1>
          </div>
        </div>
        ...
        <div className="px-6 mb-8">
           <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
              <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-1">On Duty Now</p>
              ...
              {loading ? (
                ...
                <>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold mt-0.5">Clinical Specialist</p>
                </>
              )}
              
              {/* Subtle background icon */}
              <LayoutDashboard className="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform duration-700" size={80} />
           </div>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => (
            ...
            >
              <div className={isActive(item.path) ? 'text-white' : 'text-blue-500/60'}>
                {item.icon}
              </div>
        ...
        <div className="p-6 border-t border-slate-100 space-y-2">
          ...
          >
            <LogOut size={18} />
            <span className="text-xs uppercase tracking-widest">Sign Out System</span>
          </button>
        </div>
      </div>
